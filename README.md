...
The "Connect a Telegram account" form doesn't ask for a session string to be
pasted in from an offline script, or generate one itself — it's generated by
scanning a QR code, via `TelegramLoginWizard`:

1. Fill in `api_id`/`api_hash` (from [my.telegram.org](https://my.telegram.org),
   entirely outside this app).
2. Click "Connect with Telegram QR" — this hits `POST /api/proxy/telegram-qr-login/start`,
   a same-origin proxy route that forwards (with the Clerk session token) to
   backend-api-service's `/api/telegram-qr-login/start`, which in turn calls
   telegram-integration-service. That service does the actual MTProto work
   via its Telethon sidecar and returns a `tg://login?token=...` deep link,
   rendered here as a QR code with `qrcode.react`.
3. Scan it with Telegram (Settings → Devices → Link Desktop Device). The
   wizard polls `/api/proxy/telegram-qr-login/{loginId}/poll` every 3s;
   if the code expires before it's scanned (~30s), the wizard silently
   fetches a fresh one via `/recreate` and keeps polling. If the account has
   a 2FA password, the wizard asks for it and submits it via `/password`.
4. Once authorized, the session string *and* phone number (Telegram reports
   both) fill the form automatically, and the submit button unlocks.

This frontend never talks to Telegram directly — no `telegram` (GramJS)
dependency, no MTProto handshake logic here at all. Every proxy route under
`src/app/api/proxy/telegram-qr-login/` just forwards to backend-api-service
the same way the other `/api/proxy/*` routes forward to `/api/tenants` etc.
See telegram-integration-service's README for the actual QR handshake, and
backend-api-service's README for how it proxies between the two.

`generate_session.py` (in telegram-integration-service) is still there as an
offline fallback if you ever want to generate a session string outside the
app entirely, but it's no longer the primary path.
