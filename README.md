...
Next.js (App Router) dashboard: sign in with Clerk, connect Telegram accounts,
...
| `/dashboard/new` | Connect a Telegram account + set its persona |
...
- **Visual direction**: clean white/light theme (off-white background, white
  surfaces, a deepened rose "spark" accent for contrast on white) rather than
  copying Telegram's own blue — this is a distinct product, not a copy of any
  one messaging app. Space Grotesk for headings, Inter for body text, IBM
  Plex Mono for timestamps/ids.
...
- **Chat bubbles**: `ASSISTANT` messages (what your number actually sent)
  ...
  incoming texts) render on the left — mirroring WhatsApp Web's familiar left/right
  convention for "them" vs "you."
- **No contact names yet** — Telegram only gives you a chat id for a new
